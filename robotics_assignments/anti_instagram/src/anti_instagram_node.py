...
class AntiInstagramNode():
    ...
    def process_image(self, image_msg):        
        try:
            ...
            self.image_lock.release()
            return
        noi = self.noisy("gauss",image)
        img_out = noi.astype(np.uint8)
#        image_out = cv2.drawKeypoints(noi,keypointList,img_out,(255,0,0),4)       
        #img = color_balanced_image1.astype(np.float32)
        color_balanced_image = self.ai.apply_color_balance(img_out, 
                                   self.output_scale)
       # x =  np.asarray(color_balanced_image1)
        #color_balanced_image = self.noisy("gauss",color_balanced_image1)
       # print (type(color_balanced_image1))
        print(type(color_balanced_image))
